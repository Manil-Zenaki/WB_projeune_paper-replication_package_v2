...
ylabel( ///
53.75 "Age of the respondent (z-score)" ///
49.75 "Number of dependent children (z-score)" /// 
45.75 "Household wealth index by cohort (z-score)" /// 
41.75 "Years of education (z-score)" /// 
...
25.75 "Network size (z-score)" /// 
21.75 "Proportion of males in the network (z-score)" /// 
17.75 "Male role model" /// 
13.75 "Female role model" /// 
9.75 "Can seek professional advice from individuals outside the family" /// 
5.75 "Gender attitudes (z-score)" /// 
1.75 "Agency: input in productive decisions (z-score)" /// 
, add ) /// styling trick
yline(3.75 7.75 11.75 15.75 19.75 23.75 27.75 31.75 /// 
35.75 39.75 43.75  47.75 51.75 , lstyle(grid)) ///
legend( order (1 "Male" 2 "Female" ) /// 
pos(6) row(1))  ///
)

graph export "$Output/short_paper/Figure_1_v4.png", replace
graph export "$Output/short_paper/Figure_1_v4.svg", replace
...
graphregion(color(white)) ///
ysize(5) xsize(9) ///
title("Combined Factors",  justification(center)) ///
ytitle("") xtitle("Regression coefficient") ///
ylabel(none, notick labsize(small) angle(horizontal) nogrid) ///
ylabel( ///
...
graph export "$Output/short_paper/Figure_1_v5.png", replace
graph save "$Output/short_paper/Figure_1_v5.gph", replace
...
graph export "$Output/short_paper/Figure_1_v6.png", replace
graph save "$Output/short_paper/Figure_1_v6.gph", replace
