...
local ga_agency_dm_labels "Attitude toward gender roles (z-score)" /// 
...
"Agrees that by nature men and women have different abilities in differenta areas" /// 
 ...
 "Attitude towards domestic violence (z-score)"
...
foreach section in `sections' { // start loop  over sections
	...
	gen interaction = gender * `var'
	
	reg train_choice_mds `var'  gender interaction ///
	i.cohort i.city ,  cluster(zone)
	
	local coeff = _b[interaction]
	local coeff : display %9.3f `coeff'
	
	local se = _se[interaction] 
	local se : display %9.3f `se'
	
	boottest interaction, level(90) weighttype(webb) seed(1947) nograph
	local p_bootstrapped = `r(p)' 
	...
	local row_se = `row' + 1 
	replace coeff_diff=  "(" + "`se'" + ")" in `row_se'
		
	
	local obs = "`e(N)'"
	replace coeff_diff = "`obs'" if label_col == "Observations"
	drop interaction
...
"\toprule" ///
"VARIABLES & Male & Female & Male = Female p-value\\*"  ///
"\midrule" ///
"\endfirsthead" ///
"%" ///
"\multicolumn{1}{c}%" ///
...
"The first two coloumns correspond to coefficient estimates from an OLS regression of training choice on respondent characteristics as specified in  \ref{eq:E1}, separately for men (first column) and women (second column). In the third column, that the  variable coefficients are equal for both regressions and show the p-value of the test. " ///
"}" ///
