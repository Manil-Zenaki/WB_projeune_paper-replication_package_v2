...
rename region city 
replace city = id8_ville if cohort > 1 
...
forval i=1/6 {
capture replace asr_value= asr_value + asr_value_`i' + " " if `i'<6 & ///
!strpos(asr_value,asr_value_`i') 
capture replace asr_value= asr_value + asr_value_`i' if `i'==6  & ///
!strpos(asr_value,asr_value_`i') 
}
...
gen anr_value=""
forval i=1/8 {
capture replace anr_value= anr_value + anr_value_`i' + " " if `i'<8 & ///
!strpos(anr_value,anr_value_`i') 
capture replace anr_value= anr_value + anr_value_`i' if `i'==8  & ///
!strpos(anr_value, anr_value_`i') 
}
...
gen ara_value=""
forval i=1/3 {
capture replace ara_value= ara_value + ara_value_`i' + " " if `i'<3 & ///
!strpos(ara_value,ara_value_`i') 
capture replace ara_value= ara_value + ara_value_`i' if `i'==3  & ///
!strpos(ara_value,ara_value_`i') 
}
...
forval i=1/29 {
capture replace train_field= train_field +  "`i'" + " " if `i'<29 & ///
...
gen ca1_`x'_cat =. // =1 if respondent involved in the decision
replace  ca1_`x'_cat=1 if inlist(ca1_`x', 3, 7, 9 ) |  /// joint decision 
(strpos(ca1_oth_`x', "MOI") & strpos(ca1_oth_`x', "ET") )
replace  ca1_`x'_cat= 2 if ca1_`x'==1   | /// 
(strpos(ca1_oth_`x', "MOI") &  !strpos(ca1_oth_`x', "ET") ) // sole decision 
...
replace  ca1_`x'_dummy=1 if inlist(ca1_`x', 1, 3, 7, 9 ) | strpos(ca1_oth_`x', "MOI")
